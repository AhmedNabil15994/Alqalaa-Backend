...
trait CrudDashboardController
{
    ...
    protected $currentExportTransaction = null;
    ...
    {
        $transaction = PrintReportsRequest::create(['user_id' => auth()->user()->id,'status' => 0]);
        PrintReport::dispatch(get_class($this),$this->handlingExportRequest($request)->all(),$type,$transaction,locale());
    ...
    public function exportTransaction($request, $type, $transaction)
    {
        ...
        switch ($type) {
            ...
            case 'excel':
                return $this->repository->exportExcel($this->repository->QueryTable($request) , $this->model_resource, $transaction);
        }
        return 'field';
    }
